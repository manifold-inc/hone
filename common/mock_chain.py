...
class MockChainInterface(ChainInterface):
    ...
    def __init__(
        ...
        logger.info(
            f"🔧 MockChainInterface initialized with {num_mock_miners} mock miners"
        )
    ...
    def _generate_mock_nodes(self) -> List[Node]:
        ...
        validator_node = Node(
            ...
            if self.keypair
            else "5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY",
        ...
        for i in range(1, self.num_mock_miners + 1):
            ...
            miner_node = Node(
                ...
                if i == 1
                    ...
                    8092 if i == 2 else 8093
                ),  # for now only support 3 test mock miners
                is_validator=False,
            )
            nodes.append(miner_node)

        logger.info(
            f"Generated {len(nodes)} mock nodes (1 validator, {self.num_mock_miners} miners)"
        )
        return nodes
    ...
    def set_weights(
        ...
        if blocks_since_last < min_interval:
            logger.warning(
                f"🔧 Mock rate limit: {blocks_since_last} blocks since last update, {min_interval} required"
            )
            return None
        ...
        for uid, weight in zip(uids, weights):
            ...
            if miner_info:
                logger.info(
                    f"  UID {uid} (port {miner_info.port}): {norm_weight:.4f} ({weight:.0f} raw)"
                )
            else:
