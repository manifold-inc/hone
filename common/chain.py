...
def load_hotkey_keypair(
    wallet_name: str, hotkey_name: str, wallet_path: Optional[str] = None
) -> Keypair:
    """Load hotkey keypair from Bittensor wallet files"""
    base_path = (
        Path(wallet_path) if wallet_path else Path.home() / ".bittensor" / "wallets"
    )
...
def query_substrate(
    ...
    try:
        ...
        logger.debug(
            f"Substrate query failed with error: {e}. Reconnecting and retrying."
        )
        substrate = SubstrateInterface(
            url=substrate.url, ss58_format=SS58_FORMAT, use_remote_preset=True
        )
        block_hash = substrate.get_block_hash(block) if block is not None else None
        query_result = substrate.query(module, method, params, block_hash=block_hash)
        return_val = query_result.value if return_value else query_result
...
def _normalize_and_quantize_weights(
    ...
    if drift != 0:
        max_idx = max(
            range(len(node_weights_formatted)), key=lambda i: node_weights_formatted[i]
        )
...
def blocks_since_last_update(
    substrate: SubstrateInterface, netuid: int, node_id: int
) -> int:
...
def min_interval_to_set_weights(substrate: SubstrateInterface, netuid: int) -> int:
    ...
    assert isinstance(weights_set_rate_limit, int), (
        "WeightsSetRateLimit should be an int"
    )
    return weights_set_rate_limit


def can_set_weights(
    substrate: SubstrateInterface, netuid: int, validator_node_id: int
) -> bool:
...
def set_node_weights(
    ...
    if commit_reveal_enabled:
        ...
        try:
            ...
            for uid, weight_tick, weight_float in zip(
                node_ids_formatted, node_weights_formatted, node_weights_float
            ):
                if weight_tick > 0:
                    logger.info(
                        f"  UID {uid:>3} - Ticks: {weight_tick:>5} - Normalized: {weight_float:.8f}"
                    )
...
class ChainInterface:
    ...
    def query_substrate(
        ...
        assert self.substrate, "Call connect() first"
        params = params or []
        substrate, value = query_substrate(
            self.substrate, module, method, params, return_value=True, block=block
        )
