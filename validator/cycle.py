...
async def run_query_cycle(validator, state):
    ...
    # Discover miners once at the start of the cycle
    ...
    # Query continuously for CYCLE_DURATION blocks
    queries_in_cycle = 0
    while True:
        current_block = validator.get_current_block()
        ...
        # Wait a bit before next query round (e.g., 10 seconds)
...
async def run_weights_cycle(validator, state):
    ...
    # Check if it's time to set weights (after each query cycle)
...
async def run_continuous(validator, stop_event: asyncio.Event = None):
    ...
    while True:
        ...
        try:
            # Run a full query cycle
            await run_query_cycle(validator, validator.state)
            
            # After query cycle completes, set weights
            ...
            # Short wait before starting next cycle
