...
async def run_query_cycle(validator, state):
    ...
    if (
        state["last_query_block"]
        and (current_block - state["last_query_block"])
        < validator.config.query_interval_blocks
    ):
        logger.debug(
            f"Skipping query cycle - only {current_block - state['last_query_block']} blocks since last query"
        )
        return
...
async def run_weights_cycle(validator, state):
    ...
    if (
        state["last_weights_block"]
        and (current_block - state["last_weights_block"])
        < validator.config.weights_interval_blocks
    ):
        logger.debug(
            f"Skipping weights cycle - only {current_block - state['last_weights_block']} blocks since last weights"
        )
        return
    ...
    scores, hotkey_map = await scoring.calculate_scores_and_update_leaderboard(
        validator.db, validator.config, miners
    )
...
async def run_continuous(validator, stop_event: asyncio.Event = None):
    ...
    logger.info(
        f"  Max submissions per day: {validator.config.max_submissions_per_day}"
    )
    logger.info(
        f"  Min accuracy floor: {validator.config.min_accuracy_floor * 100:.1f}%"
    )
    ...
    while True:
        ...
        try:
            ...
            logger.info(
                f"Completed cycle {validator.state['cycle_count']}, waiting before next cycle..."
            )
