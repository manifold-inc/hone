...
async def calculate_scores_and_update_leaderboard(
    db, config, miners: Dict[int, Dict]
    ...
    rows = await db.get_recent_results(
        window_blocks=window_blocks, current_block=current_block
    )

    registered_hotkeys = {
        miner.get("hotkey") for miner in miners.values() if miner.get("hotkey")
    }
    ...
    for r in rows:
        ...
        if success:
            ...
            exact_match_rate = (
                r.get("exact_match_rate")
                if hasattr(r, "get")
                else r["exact_match_rate"]
            )
    ...
    for hotkey, stats in miner_stats.items():
        if stats["count"] < min_responses:
            logger.debug(
                f"Hotkey {hotkey[:12]}...: only {stats['count']} responses < min_responses={min_responses}"
            )
            continue
        ...
        avg_exact_match_rate = (
            stats["exact_match_rate_sum"] / stats["successful_responses"]
        )

        if avg_exact_match_rate < min_accuracy_floor:
            logger.debug(
                f"Hotkey {hotkey[:12]}...: Below floor ({avg_exact_match_rate * 100:.1f}% < {min_accuracy_floor * 100:.1f}%)"
            )
            continue
    ...
    for hotkey, uid, exact_match_rate in top_miners:
        ...
        if miner:
            recent_results = await db.get_recent_results(
                window_blocks=window_blocks, current_block=current_block
            )
            repo_info = None
            ...
            if repo_info:
                await db.update_leaderboard(
                    ...
                    if hasattr(repo_info, "get")
                    else repo_info["repo_url"],
                    repo_branch=repo_info.get("repo_branch", "main")
                    if hasattr(repo_info, "get")
                    else repo_info["repo_branch"] or "main",
                    repo_commit=repo_info.get("repo_commit")
                    if hasattr(repo_info, "get")
                    else repo_info["repo_commit"],
                    repo_path=repo_info.get("repo_path", "")
                    if hasattr(repo_info, "get")
                    else repo_info["repo_path"] or "",
                )
    ...
    for hotkey in leaderboard_hotkeys - top_hotkeys:
        logger.info(
            f"Removing {hotkey[:12]}... from leaderboard (no longer in top {top_miners_count})"
        )
    ...
    scores_for_db = {
        uid: exact_match_rate for hotkey, uid, exact_match_rate in qualifying_miners
    }
    ...
    logger.info(f"=" * 80)
    logger.info(
        f"Leaderboard updated: {len(top_miners)} miners in top {top_miners_count}"
    )
...
async def set_weights(chain, config, scores: Dict[int, float]) -> bool:
    ...
    if not has_qualifying_miners:
        ...
        logger.info("=" * 60)
        logger.info(
            f"Weight distribution: {BURN_PERCENTAGE * 100:.0f}% burn + {MINER_PERCENTAGE * 100:.0f}% to miners"
        )
        logger.info(
            f"  UID {BURN_UID:>3} - Weight: {BURN_PERCENTAGE * 100:>6.2f}% (burn)"
        )
    ...
    try:
        result = chain.set_weights(uids=all_uids, weights=all_weights)
...
async def check_can_set_weights(chain, config) -> bool:
    ...
    try:
        can_set = can_set_weights(chain.substrate, chain.netuid, chain.validator_uid)
