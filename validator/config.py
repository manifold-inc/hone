...
def resolve_hotkey(
    wallet_name: str | None, wallet_hotkey: str | None, wallet_path: str | None
) -> Optional[str]:
    """
...
class ValidatorConfig:
    ...
    hotkey: Optional[str] = resolve_hotkey(
        wallet_name=wallet_name, wallet_hotkey=wallet_hotkey, wallet_path=wallet_path
    )
    ...
    _cycle_duration: int = field(
        default_factory=lambda: int(os.getenv("CYCLE_DURATION", "30"))
    )
    ...
    sandbox_max_poll_attempts: int = int(
        os.getenv("SANDBOX_MAX_POLL_ATTEMPTS", "360")
    ...
    top_miners_count: int = int(os.getenv("TOP_MINERS_COUNT", "5"))
