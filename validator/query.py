...
async def fetch_miner_info(
    ...
    try:
        async with session.get(url, timeout=timeout) as resp:
            ...
            logger.debug(
                f"UID {uid}: Fetched info - repo: {info.get('repo_url')}, weight: {info.get('weight_class')}"
            )
            return info
...
async def query_miners_via_sandbox(
    ...
    for uid, miner in miners.items():
        ...
        if not can_submit:
            logger.info(
                f"UID {uid} ({hotkey[:12]}...): Daily limit reached ({current_count}/{config.max_submissions_per_day})"
            )
            continue

        eligible_miners[uid] = miner
        logger.debug(
            f"UID {uid} ({hotkey[:12]}...): Eligible ({current_count}/{config.max_submissions_per_day} used)"
        )
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        tasks = [
            fetch_miner_info(
                ...
                uid,
                miner,
                config.default_miner_port,
                config.miner_info_timeout_seconds,
            )
    ...
    for uid, info in miner_infos.items():
        ...
        if history:
            logger.info(
                f"UID {uid} ({hotkey[:12]}...): Using cached metrics (evaluated {history['evaluation_count']} times)"
            )
    ...
    for uid, job_data in jobs_to_submit.items():
        try:
            ...
            logger.info(
                f"UID {uid}: Job {job_id} submitted (queue position: {queue_position})"
            )
    ...
    async def poll_and_record(uid: int, job_info: Dict):
        ...
        def on_status_change(job_id, status, full_status):
            ...
            logger.info(
                f"UID {uid} | Job {job_id} | {status} | {phase} | {progress:.1f}%"
            )
        ...
        if metrics_data and metrics_data.get("metrics"):
            ...
            exact_match_rate = float(aggregate.get("exact_match_rate", 0.0))

            logger.info(
                f"UID {uid} | Job {job_id} | Metrics received: exact_match_rate={exact_match_rate}"
            )
            ...
            error_msg = final_status.get(
                "error_message", f"Job status: {final_status.get('status')}"
            )
    ...
    logger.info(
        f"Polling {len(job_submissions)} jobs (timeout: {config.sandbox_runner_timeout_hours}h)..."
    )

    tasks = [
        poll_and_record(uid, job_info) for uid, job_info in job_submissions.items()
    ]
