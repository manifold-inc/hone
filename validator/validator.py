...
class Validator:
    ...
    def get_current_block(self) -> int:
        try:
            ...
            logger.warning(
                f"Could not read current block from chain ({e}); falling back to 0"
            )
            return 0
    ...
    async def maybe_cleanup_database(self):
        ...
        if (
            self.last_cleanup_time is None
            or current_time - self.last_cleanup_time > cleanup_interval_seconds
        ):
            logger.info(
                f"Running database cleanup (retention: {self.config.retention_days} days)"
            )
