        ...
        if len(self.cursors) > 100:
            ...
            if len(self.cursors) > keep_count:
                sorted_cursors = sorted(
                    self.cursors.items(), key=lambda x: x[1], reverse=True
                )
...
class LogManager:
    ...
    def append_log(self, job_id: str, phase: str, message: str, level: str = "info"):
        """
    ...
    def append_logs_batch(
        self, job_id: str, phase: str, messages: List[str], level: str = "info"
    ):
