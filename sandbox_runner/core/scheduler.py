...
class IntelligentScheduler:
    ...
    async def _check_for_starvation(
        self, jobs: List[Job], available_gpus: int
    ) -> Optional[Job]:
        """
    ...
    async def _find_optimal_job(
        ...
        for job in jobs:
            ...
            score = self._calculate_job_score(job, required_gpus, available_gpus)
    ...
    def _calculate_job_score(
        self, job: Job, required_gpus: int, available_gpus: int
    ) -> float:
        """
    ...
    async def get_scheduling_recommendation(self) -> Dict:
        ...
        if not jobs:
            return {"recommendation": "no_jobs", "message": "No jobs in queue"}

        if available_gpus == 0:
            return {"recommendation": "no_gpus", "message": "No GPUs available"}
