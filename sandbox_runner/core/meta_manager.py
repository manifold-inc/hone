...
class MetaManager:
    ...
    async def start(self):
        ...
        self._dataset_generation_task = asyncio.create_task(
            self._dataset_generation_loop()
        )
        ...
        while self._running:
            try:
                if await self.dataset_manager.should_generate_today():
                    ...
                    logger.info(
                        "All job submissions will be queued until generation completes"
                    )
                    logger.info("=" * 60)
    ...
    async def get_runner_status(self) -> Dict:
        ...
        pending_execution = {
            jid for jid in self._running_jobs if jid not in executor_active
        }
        ...
        for job_id in completed_jobs:
            ...
            if not job:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to release GPUs for missing job {job_id}: {e}"
                    )
                continue
    ...
    async def get_queue_breakdown(self) -> Dict[str, Dict]:
        ...
        for weight_class in WeightClass:
            class_jobs = [job for job in all_jobs if job.weight_class == weight_class]
    ...
    async def get_job_logs(
        self, job_id: str, lines: int = 100, offset: int = 0
    ) -> Optional[Dict]:
        """Get job logs"""
        job = None
                ...
                try:
                    ...
                    if containers:
                        ...
                        log_output = container.logs(
                            stdout=True, stderr=True, tail=lines
        ...
        if job.started_at:
            logs.extend(
                [
                    ...
                    {
                        "timestamp": (
                            job.started_at + timedelta(seconds=5)
                        ).isoformat(),
                        "level": "INFO",
                    ...
                    {
                        "timestamp": (
                            job.started_at + timedelta(seconds=30)
                        ).isoformat(),
                        "level": "INFO",
