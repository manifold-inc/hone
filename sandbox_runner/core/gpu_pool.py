...
class GPUPoolManager:
    ...
    async def allocate_gpus(
        self, weight_class: WeightClass, job_id: str
    ) -> Optional[List[int]]:
        """
        ...
        async with self._lock:
            ...
            available_count = sum(
                1 for gpu in self._gpus.values() if gpu.status == GPUStatus.FREE
            )
    ...
    async def get_available_gpu_count(self) -> int:
        ...
        """
        async with self._lock:
            return sum(1 for gpu in self._gpus.values() if gpu.status == GPUStatus.FREE)
    ...
    async def get_allocation_stats(self) -> Dict:
        ...
        async with self._lock:
            ...
            allocated_count = sum(
                1 for gpu in self._gpus.values() if gpu.status == GPUStatus.ALLOCATED
            )
    ...
    def _find_contiguous_gpus(self, count: int) -> Optional[List[int]]:
        ...
        for start_id in range(self.gpu_count - count + 1):
            ...
            if all(self._gpus[gpu_id].status == GPUStatus.FREE for gpu_id in gpus):
                return gpus
    ...
    def _find_any_free_gpus(self, count: int) -> Optional[List[int]]:
        ...
        """
        free_gpus = [
            gpu_id for gpu_id, gpu in self._gpus.items() if gpu.status == GPUStatus.FREE
        ]
