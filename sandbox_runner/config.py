...
class NetworkPolicyConfig:
    ...
    allowed_prep_domains: list[str] = field(
...
def load_config(config_path: Path) -> Config:
    ...
    if "execution" in yaml_data:
        ...
        if "SHOW_TERMINAL_LOGS" in os.environ:
            exec_data["show_terminal_logs"] = (
                os.environ["SHOW_TERMINAL_LOGS"].lower() == "true"
            )
        ...
        if "LOG_RETENTION_HOURS" in os.environ:
            try:
                exec_data["log_retention_hours"] = int(
                    os.environ["LOG_RETENTION_HOURS"]
                )
            except ValueError:
    ...
    if "monitoring" in yaml_data:
        ...
        if (
            "falco_rules_path" in monitoring_data
            and monitoring_data["falco_rules_path"]
        ):
            monitoring_data["falco_rules_path"] = Path(
                monitoring_data["falco_rules_path"]
            )
...
def _validate_config(config: Config):
    ...
    if config.execution.mode not in valid_modes:
        raise ValueError(
            f"Invalid execution mode: {config.execution.mode}. Must be one of {valid_modes}"
        )

    if config.hardware.gpu_count < 1:
        raise ValueError(
            f"GPU count must be at least 1, got {config.hardware.gpu_count}"
        )

    if not (1024 <= config.api.port <= 65535):
        raise ValueError(
            f"API port must be between 1024 and 65535, got {config.api.port}"
        )

    if config.execution.cpu_limit < 1:
        raise ValueError(
            f"CPU limit must be at least 1, got {config.execution.cpu_limit}"
        )

    if config.execution.memory_limit_gb < 1:
        raise ValueError(
            f"Memory limit must be at least 1GB, got {config.execution.memory_limit_gb}"
        )

    if config.execution.prep_timeout_seconds < 60:
        raise ValueError(
            f"Prep timeout must be at least 60 seconds, got {config.execution.prep_timeout_seconds}"
        )

    if config.execution.inference_timeout_seconds < 60:
        raise ValueError(
            f"Inference timeout must be at least 60 seconds, got {config.execution.inference_timeout_seconds}"
        )

    if config.execution.log_retention_hours < 0:
        raise ValueError(
            f"Log retention must be at least 0 hours, got {config.execution.log_retention_hours}"
        )
...
def generate_default_config(output_path: Path):
    ...
    default_config = {
        ...
        "security": {
            "seccomp": {"enabled": True, "default_action": "SCMP_ACT_ERRNO"},
            "network_policy": {
                "prep_allow_internet": True,
                "inference_block_internet": True,
                "allowed_prep_domains": ["huggingface.co", "github.com", "gitlab.com"],
            },
            "readonly_rootfs": True,
            "no_new_privileges": True,
            "drop_capabilities": ["CAP_SYS_ADMIN", "CAP_NET_ADMIN", "CAP_SYS_MODULE"],
        },
        "monitoring": {
