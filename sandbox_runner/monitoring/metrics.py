...
class MetricsManager:
    ...
    def _init_system_metrics(self):
        ...
        self.system_info = Info(
            "sandbox_system", "System information", registry=self.registry
        )

        self.cpu_usage = Gauge(
            "sandbox_cpu_usage_percent", "CPU usage percentage", registry=self.registry
        )

        self.memory_used = Gauge(
            "sandbox_memory_used_bytes", "Memory used in bytes", registry=self.registry
        )
    ...
    def initialize(self):
        ...
        self.system_info.info(
            {"version": "1.0.0", "service": "hone-subnet-sandbox-runner"}
        )
    ...
    def record_job_submitted(self, validator: str, weight_class: str):
        ...
        self.jobs_submitted.labels(validator=validator, weight_class=weight_class).inc()

    def record_job_completed(
        self, status: str, weight_class: str, validator: str, duration: float
    ):
        """Record a job completion with duration"""
        self.jobs_completed.labels(
            status=status, weight_class=weight_class, validator=validator
        ).inc()

        self.job_duration.labels(phase="total", weight_class=weight_class).observe(
            duration
        )

    def record_job_phase_duration(self, phase: str, weight_class: str, duration: float):
        """Record duration of a specific job phase"""
        self.job_duration.labels(phase=phase, weight_class=weight_class).observe(
            duration
        )
    ...
    def record_auth_failure(self, auth_type: str, reason: str):
        ...
        self.auth_failures.labels(auth_type=auth_type, reason=reason).inc()
