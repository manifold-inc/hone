...
if [ -f .gitignore ]; then
    if ! grep -q ".streamlit/secrets.toml" .gitignore; then
        ...
        echo -e "${GREEN}✓${NC} Added secrets.toml to .gitignore"
    fi
else
    echo ".streamlit/secrets.toml" > .gitignore
    echo -e "${GREEN}✓${NC} Created .gitignore with secrets.toml"
fi

echo ""
echo -e "${BLUE}3. Checking dashboard files...${NC}"

if [ -f dashboard.py ]; then
    echo -e "${GREEN}✓${NC} dashboard.py found"
else
    echo -e "${YELLOW}⚠${NC}  dashboard.py not found - make sure to copy it to your project root"
fi

if [ -f dashboard_routes.py ]; then
    echo -e "${GREEN}✓${NC} dashboard_routes.py found"
else
    echo -e "${YELLOW}⚠${NC}  dashboard_routes.py not found - copy to api/ directory"
fi

if [ -f meta_manager_extensions.py ]; then
    echo -e "${GREEN}✓${NC} meta_manager_extensions.py found"
else
    echo -e "${YELLOW}⚠${NC}  meta_manager_extensions.py not found"
fi
