...
class DirectExecutor:
    ...
    async def run_container(
        ...
        try:
            ...
            logger.error(
                f"Direct execution timeout after {timeout_seconds}s: {job_name}"
            )
            if process:
                self._kill_process(process)
            raise DirectExecutionError(f"Execution timeout after {timeout_seconds}s")
