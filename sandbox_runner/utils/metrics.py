...
def calculate_metrics_for_prediction(
    ...
    if predicted_output is None:
        return {
            ...
            "expected_shape": (
                len(expected_output),
                len(expected_output[0]) if expected_output else 0,
            ),
        }

    exact_match = predicted_output == expected_output
    partial_correctness = calculate_partial_correctness(
        predicted_output, expected_output
    )
    grid_similarity = calculate_grid_similarity(predicted_output, expected_output)

    shape_match = (
        len(predicted_output) == len(expected_output)
        and len(predicted_output[0]) == len(expected_output[0])
    ...
    return {
        ...
        "predicted_shape": (
            ...
            len(predicted_output[0]) if predicted_output else 0,
        ),
        "expected_shape": (
            len(expected_output),
            len(expected_output[0]) if expected_output else 0,
...
def calculate_aggregate_metrics(predictions: List[Dict]) -> Dict:
    ...
    solved_predictions = [
        p for p in predictions if p.get("predicted_output") is not None
    ]
    ...
    avg_partial_correctness = sum(m["partial_correctness"] for m in metrics_list) / len(
        metrics_list
    )
    avg_grid_similarity = sum(m["grid_similarity"] for m in metrics_list) / len(
        metrics_list
    )
    ...
    return {
        ...
        "shape_match_rate": round(num_shape_matches / len(metrics_list), 4)
        if metrics_list
        else 0.0,
        "success_rate": round(num_solved / total_problems, 4),
        "exact_match_rate": round(num_exact_matches / len(metrics_list), 4)
        if metrics_list
        else 0.0,
    }
...
def calculate_detailed_metrics(results_data: Dict) -> Dict:
    ...
    return {"aggregate": aggregate, "per_problem": per_problem_metrics}
