...
class RepositoryValidator:
    ...
    OPTIONAL_FILES = ["Dockerfile", "README.md", ".gitignore"]
    ...
    def validate_structure(self, repo_path: Path) -> bool:
        ...
        for required_file in self.REQUIRED_FILES:
            ...
            if not file_path.exists():
                available_files = (
                    list(repo_path.iterdir()) if repo_path.exists() else []
                )
                logger.error(
                    f"Available files in {repo_path}: {[f.name for f in available_files]}"
                )
        ...
        self._check_file_size(
            repo_path / "arc_main.py", self.MAX_INFERENCE_SIZE, "arc_main.py"
        )
        self._check_file_size(
        ...
        self._check_file_size(
            repo_path / "Dockerfile", self.MAX_INFERENCE_SIZE, "Dockerfile"
        )
        self._check_file_size(
        ...
        if dockerfile_path.exists():
            self._check_file_size(
                dockerfile_path, self.MAX_DOCKERFILE_SIZE, "Dockerfile"
            )
