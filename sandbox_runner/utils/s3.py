...
class S3Manager:
    ...
    def __init__(self, config: StorageConfig):
        ...
        if config.s3_access_key and config.s3_secret_key:
            ...
            self.s3_client = boto3.client(
                "s3", endpoint_url=config.s3_endpoint, config=boto_config
            )
    ...
    async def download_input_data(
        self, s3_path: str, local_path: Path, max_retries: int = 3
    ) -> bool:
        """
        ...
        if self._is_presigned_url(s3_path):
            ...
            return await self._download_from_s3_uri(s3_path, local_path, max_retries)

    async def _download_from_presigned_url(
        self, presigned_url: str, local_path: Path, max_retries: int = 3
    ) -> bool:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(presigned_url) as response:
                        if response.status == 200:
                            ...
                            logger.info(
                                f"Downloaded {file_size} bytes from presigned URL"
                            )
                            return True
        ...
        for attempt in range(max_retries):
            try:
                await asyncio.get_event_loop().run_in_executor(
                    None, self._download_file, bucket, key, local_path
                )
    ...
    async def upload_output_data(
        self, local_path: Path, s3_path: str, max_retries: int = 3
    ) -> bool:
        """
        ...
        if self._is_presigned_url(s3_path):
            return await self._upload_to_presigned_url(local_path, s3_path, max_retries)
        else:
            return await self._upload_to_s3_uri(local_path, s3_path, max_retries)

    async def _upload_to_presigned_url(
        self, local_path: Path, presigned_url: str, max_retries: int = 3
    ) -> bool:
                    ...
                    with open(local_path, "rb") as f:
                        async with session.put(presigned_url, data=f) as response:
                            if response.status in [200, 204]:
                                logger.info(
                                    f"Uploaded {file_size} bytes to presigned URL"
                                )
                                return True
                            else:
                                raise S3TransferError(
    ...
    async def _upload_to_s3_uri(
        ...
        for attempt in range(max_retries):
            try:
                await asyncio.get_event_loop().run_in_executor(
                    None, self._upload_file, local_path, bucket, key
                )
    ...
    async def download_directory(
        self, s3_prefix: str, local_dir: Path, max_retries: int = 3
    ) -> bool:
        ...
        if self._is_presigned_url(s3_prefix):
            ...
            return await self._download_from_presigned_url(
                s3_prefix, local_dir / filename, max_retries
            )
        ...
        try:
            objects = await asyncio.get_event_loop().run_in_executor(
                None, self._list_objects, bucket, prefix
            )
            ...
            for obj in objects:
                ...
                task = self._download_from_s3_uri(
                    f"s3://{bucket}/{key}", local_path, max_retries
                )
    ...
    async def upload_directory(
        self, local_dir: Path, s3_prefix: str, max_retries: int = 3
    ) -> bool:
        ...
        for local_path in files:
            ...
            task = self._upload_to_s3_uri(
                local_path, f"s3://{bucket}/{key}", max_retries
            )
