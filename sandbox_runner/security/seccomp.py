    ...
    def to_docker_seccomp(self) -> Dict:
        ...
        if self.allowed_syscalls:
            profile["syscalls"].append(
                {"names": self.allowed_syscalls, "action": "SCMP_ACT_ALLOW"}
            )

        if self.blocked_syscalls:
            profile["syscalls"].append(
                {"names": self.blocked_syscalls, "action": "SCMP_ACT_ERRNO"}
            )
...
class SeccompManager:
    ...
    def _create_default_profiles(self):
        ...
        self.profiles["strict"] = SeccompProfile(
            default_action="SCMP_ACT_ERRNO", allowed_syscalls=self.ESSENTIAL_SYSCALLS
        )
        ...
        self.profiles["permissive"] = SeccompProfile(
            default_action="SCMP_ACT_ALLOW", blocked_syscalls=self.DANGEROUS_SYSCALLS
        )
    ...
    def is_syscall_allowed(self, profile_name: str, syscall_name: str) -> bool:
        """
