...
class NamespaceManager:
    ...
    def _check_unshare_available(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["which", "unshare"], capture_output=True, timeout=5
            )
    ...
    def create_network_namespace(self, namespace_name: str) -> bool:
        ...
        try:
            result = subprocess.run(
                ["ip", "netns", "add", namespace_name], capture_output=True, timeout=10
            )
    ...
    def exec_in_network_namespace(
        self, namespace_name: str, command: List[str]
        ...
        try:
            ...
            result = subprocess.run(netns_command, capture_output=True, timeout=30)
    ...
    def isolate_process(
        self, pid: int, network: bool = True, pid_ns: bool = False
    ) -> bool:
        """
        ...
        """
        if pid_ns:
            logger.warning("Cannot change PID namespace of existing process")
    ...
    def check_namespace_isolation(
        self, pid1: int, pid2: int, namespace_type: str = "net"
    ) -> bool:
        """
