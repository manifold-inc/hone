...
class NetworkPolicy:
    ...
    async def enable_internet_for_container(
        self, container_id: str, phase: str = "prep"
    ) -> bool:
    ...
    async def block_internet_for_container(
        self, container_id: str, phase: str = "inference"
    ) -> bool:
    ...
    async def setup_domain_filtering(self, container_id: str, phase: str = "prep"):
    ...
    def get_policy_summary(self, phase: str) -> dict:
        ...
        internet_enabled = (phase == "prep" and self.config.prep_allow_internet) or (
...
class IptablesNetworkPolicy(NetworkPolicy):
    ...
    def __init__(self, config: NetworkPolicyConfig):
        ...
        logger.info(
            f"IptablesNetworkPolicy initialized (available={self.iptables_available})"
        )
    ...
    async def setup_container_network_isolation(
        self, container_id: str, phase: str
    ) -> bool:
        """
        ...
        try:
            ...
            if not netns_pid:
                logger.warning(
                    f"Could not get PID for {container_id[:12]}, skipping iptables setup"
                )
                return False
            ...
            logger.info(
                f"Network isolation configured for {container_id[:12]} (phase={phase})"
            )
            return True
    ...
    async def monitor_connections(
        ...
        if not pid:
            logger.warning(
                f"Cannot monitor container {container_id[:12]}: no PID found"
            )
            return []
        ...
        try:
            ...
            logger.info(
                f"Monitored {len(connections)} connection attempts for {container_id[:12]}"
            )
            return connections
