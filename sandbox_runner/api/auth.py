...
class AuthenticationManager:
    ...
    def __init__(self, config: APIConfig):
        ...
        """
        self.config = config
        self.api_key_scheme = APIKeyHeader(name=config.api_key_header, auto_error=False)
    ...
    async def verify_api_key(self, api_key: Optional[str] = None) -> str:
        """
...
class RateLimiter:
    ...
    async def check_rate_limit(self, identifier: str):
        ...
        self._request_log[identifier] = [
            timestamp
            for timestamp in self._request_log[identifier]
