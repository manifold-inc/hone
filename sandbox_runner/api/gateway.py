...
async def lifespan(app: FastAPI):
    ...
    if app.state.config.execution.log_retention_hours > 0:
        log_manager = initialize_log_manager(
            app.state.config.execution.log_retention_hours
        )
        await log_manager.start()
        logger.info(
            f"LogManager initialized with {app.state.config.execution.log_retention_hours}h retention"
        )
...
def create_app(config: Config) -> FastAPI:
    ...
    if config.execution.log_retention_hours > 0:
        ...
        logger.info(
            f"Log routes enabled (retention: {config.execution.log_retention_hours}h)"
        )
    else:
    ...
    async def metrics():
        ...
        return JSONResponse(
            content=metrics_data.decode("utf-8"), media_type=content_type
        )
