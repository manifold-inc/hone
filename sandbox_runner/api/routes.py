...
class JobSubmitRequest(BaseModel):
    ...
    repo_branch: str = Field(default="main", description="Git branch", example="main")

    repo_commit: Optional[str] = Field(
        None, description="Specific commit hash (optional)", example="abc123"
    )
    ...
    weight_class: WeightClass = Field(
        ..., description="GPU weight class", example="1xH200"
    )

    priority: int = Field(
        default=0, ge=0, le=10, description="Job priority (0-10)", example=5
    )

    validator_hotkey: Optional[str] = Field(
        None, description="Validator's hotkey", example="5Abc...xyz"
    )

    miner_hotkey: str = Field(..., description="Miner's hotkey", example="5Def...uvw")
    ...
    use_vllm: bool = Field(
        default=False, description="Whether to use vLLM for inference", example=False
    )
...
def create_router(config: Config) -> APIRouter:
    ...
    async def submit_job(
        ...
        return JobSubmitResponse(
            ...
            estimated_start_time=datetime.fromisoformat(
                response["estimated_start_time"]
            )
            if response.get("estimated_start_time")
            else None,
    ...
    @router.get(
        "/jobs/{job_id}/metrics", response_model=Dict, summary="Get job metrics"
    )
    ...
    @router.delete("/jobs/{job_id}", summary="Cancel a job")
    ...
    @router.get("/status", response_model=RunnerStatus, summary="Get runner status")
