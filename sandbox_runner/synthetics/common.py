...
def conway_sprite(width=3, height=3, tries=5):
    ...
    for _ in range(tries):
        ...
        if rows.count(r) <= 1 or cols.count(c) <= 1:
            continue  # would vanish!
        del rows[idx]
        del cols[idx]
    ...
    while len(pixels) < size:
        ...
        for dr, dc in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
            ...
            if r < 0 or r >= height or c < 0 or c >= width:
                continue
            if (r, c) in pixels or (r, c) in queue:
                continue
            queue.append((r, c))
    return pixels
...
def grid_intersect(width, height, rows, cols, b, s, y, n, m):
    ...
    for r in range(height):
        for c in range(width):
            if ingrid[r][c] == n or ingrid[r][c + width + 1] == n:
                continue
            output[r][c] = m
    return ingrid, output
...
def has_neighbor(size, bitmap, r, c):
    for dr in [-1, 0, 1]:
        for dc in [-1, 0, 1]:
            if r + dr < 0 or r + dr >= size or c + dc < 0 or c + dc >= size:
                continue
            if bitmap[r + dr][c + dc] > 0:
                return True
    return False
...
def hollow_conway(width=3, height=3, tries=5):
    ...
    for _ in range(tries):
        idx = randint(0, len(rows) - 1)
        r, c = rows[idx], cols[idx]
        if rows.count(r) <= 1 and r in [0, height - 1]:
            continue
        if cols.count(c) <= 1 and c in [0, width - 1]:
            continue
        del rows[idx]
        del cols[idx]
    # Shuffle the lists for good measure
...
def is_surrounded(bitmap, r, c):
    for dr, dc in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
        if get_pixel(bitmap, r + dr, c + dc) <= 0:
            return False
    return True
...
def overlaps(rows, cols, wides, talls, spacing=0):
    ...
    for j in range(len(rows)):
        for i in range(j):
            if rows[i] + talls[i] + spacing <= rows[j]:
                continue
            if rows[j] + talls[j] + spacing <= rows[i]:
                continue
            if cols[i] + wides[i] + spacing <= cols[j]:
                continue
            if cols[j] + wides[j] + spacing <= cols[i]:
                continue
            result = True
    return result
...
def rectangle_nibbles(wide, tall, coldiff):
    ...
    for c in range(wide):
        for r in [0, tall - 1]:
            ...
            if r > 0 and (c == coldiff or (c == 0 and coldiff < 0)):
                continue
            if randint(0, 1):
                continue
            rows.append(r)
            cols.append(c)
            if tall < 3:
                break  # Too thin to risk taking another bite.
    return rows, cols
...
def rand_sprite(name, width, height):
    ...
    if name == "el":
        pixels = random_el(width, height)
    if name == "you":
        pixels = random_you(width, height)
    if name == "aitch":
        pixels = random_aitch(width, height)
    if xpose:
        width, height = height, width
    if xpose:
    ...
    gravity = randint(0, 3)
    if gravity in [1, 3]:
        pixels = [(height - p[0] - 1, p[1]) for p in pixels]
    if gravity in [2, 3]:
        pixels = [(p[0], width - p[1] - 1) for p in pixels]
    return pixels
