...
class DatasetManager:
    ...
    async def generate_daily_dataset(self) -> bool:
        ...
        try:
            ...
            num_needed = max(
                self.min_total_tasks - num_unsolved_kept, self.num_new_tasks
            )

            logger.info(
                f"Need to generate {num_needed} new tasks to reach {self.min_total_tasks} total"
            )
                ...
                try:
                    ...
                    if not self.storage.is_task_seen(task_hash):
                        ...
                        if len(new_tasks) % generation_batch_size == 0:
                            logger.info(
                                f"Generated {len(new_tasks)}/{num_needed} new tasks"
                            )
                ...
                while len(new_tasks) < num_needed:
                    try:
                        ...
                        if new_tasks:
                            ...
                            base_task["task_hash"] = (
                                f"{base_task['task_hash']}_dup_{len(new_tasks)}"
                            )
            ...
            for item in new_tasks:
                todays_dataset.append({**item, "source": "synthetic"})
            ...
            for item in todays_dataset:
                ...
                validation_dataset.append(
                    {**item, "expected_output": item.get("test_output")}
                )
            ...
            for difficulty, count in difficulty_stats.items():
                logger.info(
                    f"  - {difficulty}: {count} tasks ({count * 100 / total_tasks:.1f}%)"
                )
            logger.info("=" * 60)
