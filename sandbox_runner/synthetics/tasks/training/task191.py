...
def generate(rows=None, cols=None, brow=None, bcol=None, wide=None, tall=None, size=23):
    ...
    def draw(grid, output):
        ...
        for mrow in range(size):
            for mcol in range(size):
                    ...
                    for xpose in [0, 1]:
                        ...
                        for r in range(tall):
                            for c in range(wide):
                                ...
                                if mcolor == common.blue():
                                    mcolor = common.black()
                                if bcolor == common.blue():
                                    bcolor = common.black()
                                if mcolor != bcolor:
                                    match = False
                        if match:
                ...
                if value != -1:
                    matches.append((mrow, mcol, value, xalue))
        ...
        for match in matches:
            ...
            for r in range(mrow - 1, mrow + t + 1):
                for c in range(mcol - 1, mcol + w + 1):
                    if common.get_pixel(output, r, c) == common.blue():
                        illegal = True
                    if common.get_pixel(grid, r, c) == common.yellow():
                        if r in [mrow - 1, mrow + t] or c in [mcol - 1, mcol + w]:
                            illegal = (
                                True  # There's a pixel on the border of the blue square
                            )
...
def validate():
    ...
    train = [
        generate(
            rows=[1, 2, 3, 4, 4, 5, 7, 8, 12, 13, 13, 14, 14, 15, 18, 20, 21],
            cols=[13, 4, 6, 4, 20, 18, 7, 14, 4, 11, 13, 4, 19, 12, 18, 3, 11],
            brow=2,
            bcol=4,
            wide=3,
            tall=3,
