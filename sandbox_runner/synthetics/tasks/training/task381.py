    ...
    def draw(grid, output):
        ...
        for r in range(size):
            ...
            for c in range(size):
                if output[r][c] != common.red():
                    danger = (
                        danger or common.get_pixel(output, r - 1, c) == common.red()
                    )
                    danger = (
                        danger or common.get_pixel(output, r + 1, c) == common.red()
                    )
        ...
        for r in range(1, size - 1):
            ...
            for color in output[r]:
                if state == 0 and color == common.red():
                    state = 1
                if state == 1 and color == common.black():
                    state = 2
                if state == 2 and color == common.red():
                    return False
        return True
    ...
    if rows is None:
        ...
        while True:
            ...
            if not draw(grid, output):
                continue
            if not common.overlaps(rows, cols, wides, talls, 1):
                break
...
def validate():
    ...
    train = [
        generate(rows=[2, 3, 6], cols=[0, 7, 3], wides=[3, 2, 2], talls=[3, 5, 4]),
        generate(
            rows=[0, 2, 5, 8], cols=[0, 7, 3, 6], wides=[2, 3, 2, 4], talls=[4, 4, 4, 2]
        ),
        generate(
            rows=[0, 1, 3, 6, 7],
            cols=[6, 0, 5, 9, 0],
    ...
    test = [
        generate(
            rows=[0, 1, 3, 5], cols=[0, 6, 1, 5], wides=[3, 4, 3, 4], talls=[2, 3, 5, 2]
        ),
    ]
