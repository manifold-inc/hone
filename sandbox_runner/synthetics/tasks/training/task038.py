...
def generate(rows=None, cols=None, lengths=None, colors=None, size=9):
    ...
    if rows is None:
        ...
        while True:
            ...
            if not common.overlaps(rows, cols, lengths, lengths, 1):
                break
        ...
        for r in range(size):
            for c in range(size):
                if grid[r][c] or common.randint(0, 2):
                    continue  # Something's here.
                color = common.red() if common.randint(0, 1) else common.blue()
                touching = False
                for dr in [-1, 0, 1]:
                    for dc in [-1, 0, 1]:
                        if common.get_pixel(grid, r + dr, c + dc) == color:
                            touching = True
                if touching:
                    continue
                rows.append(r)
    ...
    for row, col, length, color in zip(rows, cols, lengths, colors):
        for dr in range(length):
            for dc in range(length):
                grid[row + dr][col + dc] = color
        if length == 2 and color == common.blue():
            big_blue += 1
