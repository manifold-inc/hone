...
def generate(
    row=None, col=None, rows=None, cols=None, idxs=None, colors=None, bump=None, size=10
):
    ...
    if row is None:
        ...
        for c in range(length):
            for r in range(c + 1):
                if (r > 1 or c > 1) and common.randint(0, 1):
                    continue
                rows.append(r)
                cols.append(c)
                if length == 2:
    ...
    for r, c, idx, color in zip(rows, cols, idxs, colors):
        if idx in [-1, 0]:
            grid[row + r + bump][col + c] = color
        if idx in [-1, 1]:
            grid[row - c][col + r] = color
        if idx in [-1, 2]:
            grid[row + c + bump][col - r - bump] = color
        if idx in [-1, 3]:
            grid[row - r][col - c - bump] = color
