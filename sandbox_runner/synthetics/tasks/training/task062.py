...
def generate(
    ...
    if rows is None:
        ...
        while True:
            ...
            while queue:
                r, c = queue.pop()
                if r < 0 or r >= minisize or c < 0 or c >= minisize:
                    continue
                if bitmap[r][c] == 1:
                    continue
                bitmap[r][c] = 1
...
def validate():
    ...
    train = [
        ...
        generate(
            rows=[0, 1, 1], cols=[0, 0, 1], row=4, col=4, color=7, flip=0, xpose=0
        ),
        generate(
            rows=[0, 1, 2, 2], cols=[1, 1, 0, 1], row=3, col=4, color=8, flip=1, xpose=1
        ),
    ]
    test = [
        generate(
            rows=[0, 1, 1, 2], cols=[1, 0, 1, 0], row=3, col=4, color=1, flip=1, xpose=0
        ),
    ]
