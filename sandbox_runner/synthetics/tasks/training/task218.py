...
def generate(
    ...
    if wide is None:
        ...
        while True:  # Make sure there isn't an "L" of the same color.
            ...
            for r in range(tall):
                for c in range(wide):
                    ...
                    if r > 0 and color == patches[(r - 1) * wide + c]:
                        same += 1
                    if r < tall - 1 and color == patches[(r + 1) * wide + c]:
                        same += 1
                    if c > 0 and color == patches[r * wide + c - 1]:
                        same += 1
                    if c < wide - 1 and color == patches[r * wide + c + 1]:
                        same += 1
                    if same > 1:
                        el = True
            if el:
            ...
            for r in range(tall):
                for rp in range(r):
                    same = True
                    for c in range(wide):
                        if patches[r * wide + c] != patches[rp * wide + c]:
                            same = False
                    if same:
                        legal = False
            ...
            for c in range(wide):
                for cp in range(c):
                    same = True
                    for r in range(tall):
                        if patches[r * wide + c] != patches[r * wide + cp]:
                            same = False
                    if same:
                        legal = False
            if legal:
        ...
        while True:
            ...
            if sum(lengths) < size and sum(depths) < size:
                break
