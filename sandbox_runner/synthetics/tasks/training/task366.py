...
def generate(
        ...
        while True:
            ...
            for side in range(2):
                for idx in range(num_boxes):
                    if side != foreside and idx and not common.randint(0, 2):
                        continue
            ...
            for j in range(len(bidxs)):
                for i in range(j):
                    if bsides[i] != bsides[j]:
                        continue
                    if brows[i] + talls[bidxs[i]] < brows[j]:
                        continue
                    if brows[j] + talls[bidxs[j]] < brows[i]:
                        continue
                    if bcols[i] + wides[bidxs[i]] < bcols[j]:
                        continue
                    if bcols[j] + wides[bidxs[j]] < bcols[i]:
                        continue
                    overlaps = True
            if not overlaps:
                break
    ...
    for row in range(height):
        for col in range(width):
            r, c = row, col
            if foreside and horiz:
                c += width
            if foreside and not horiz:
                r += height
