    ...
    if width is None:
        ...
        for _ in range(3):
            if common.randint(0, 1) == 0:  # horizontal
                ...
                if cross or row in rows or col[0] in cols or col[1] in cols:
                    continue
                rows.extend([row, row])
                cols.extend(col)
                for c in range(col[0], col[1] + 1):
                ...
                if cross or row[0] in rows or row[1] in rows or col in cols:
                    continue
                rows.extend(row)
                cols.extend([col, col])
                for r in range(row[0], row[1] + 1):
        ...
        for _ in range(2):
            ...
            if bitmap[row][col] == 1 or row in rows or col in cols:
                continue
            bitmap[row][col] = 1
            rows.append(row)
            cols.append(col)
...
def validate():
    ...
    train = [
        ...
        generate(width=11, height=12, rows=[1, 4, 4, 5, 9], cols=[3, 6, 10, 1, 3]),
    ]
    test = [
        generate(
            width=13,
