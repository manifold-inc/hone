...
def generate(width=None, height=None, rows=None, cols=None, colors=None, size=10):
    ...
    if width is None:
        ...
        for r in range(3, 3 + height):
            for c in range(6 - width, 6):
                ...
                if not coords or common.randint(0, 1) == 0:
                    continue
...
def validate():
    ...
    train = [
        generate(width=2, height=4, rows=[0, 6, 9], cols=[4, 0, 5], colors=[9, 6, 4]),
        generate(
            width=3,
            height=5,
            rows=[0, 3, 5, 7, 9],
