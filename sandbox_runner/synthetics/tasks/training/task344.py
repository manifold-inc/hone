...
def generate(width=None, height=None, rows=None, cols=None, colors=None):
    ...
    if width is None:
        ...
        for pixel in common.random_pixels(width + 2, height + 2, 0.08):
            ...
            if not common.randint(0, 3):
                continue  # Rarely, we don't add a green cell
    ...
    for r, c, color in zip(rows, cols, colors):
        ...
        for dr, dc in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
            if common.get_pixel(output, r + dr, c + dc) != common.red():
                continue
...
def validate():
    ...
    train = [
        generate(width=3, height=3, rows=[0, 0, 2], cols=[0, 1, 1], colors=[3, 2, 5]),
        generate(
            width=6,
            height=7,
            rows=[0, 1, 1, 3, 3, 4, 5, 5],
            cols=[0, 2, 3, 1, 5, 1, 0, 3],
            colors=[5, 3, 2, 3, 2, 2, 5, 3],
        ),
        generate(
            width=7,
            height=7,
            rows=[0, 1, 1, 2, 2, 2, 4, 5, 5, 5, 6],
