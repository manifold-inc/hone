...
def generate(
    size=None, rows=None, cols=None, colors=None, spacing=None, linecolor=None
):
    ...
    if size is None:
        ...
        while True:
            ...
            if rows[0] in [0, size - 1] or cols[0] in [0, size - 1]:
                continue
            if not common.overlaps(rows, cols, lengths, lengths):
                break
    ...
    def draw(max_idx):
        for idx in range(len(rows)):
            ...
            if idx > max_idx:
                continue  # In the input grid, we only draw one of them.
...
def validate():
    ...
    train = [
        generate(
            size=5, rows=[1, 4], cols=[1, 3], colors=[6, 3], spacing=4, linecolor=8
        ),
        generate(
            size=7,
            rows=[2, 0, 5],
