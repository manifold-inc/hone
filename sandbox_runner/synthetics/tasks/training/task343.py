...
def generate(lengths=None, colors=None, visible=None, flip=None, width=15, height=5):
        ...
        for coloffset in range(0, width, len(lengths)):
            ...
            for dc, length in enumerate(lengths):
                for dr in range(length):
                    ...
                    if flip and flip_iter % 2:
                        c = coloffset + len(lengths) - dc - 1
                    if c < visible:
                        common.draw(grid, r, c, colors[idx])
...
def validate():
    ...
    train = [
        ...
        generate(lengths=[1, 1, 3, 1], colors=[3, 3, 2, 2, 2, 1], visible=8, flip=0),
        generate(lengths=[3, 2, 1], colors=[4, 4, 4, 3, 3, 2], visible=10, flip=1),
    ]
    test = [
        generate(
            lengths=[2, 2, 3, 1], colors=[6, 6, 6, 2, 2, 2, 2, 3], visible=10, flip=0
        ),
    ]
