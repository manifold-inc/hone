...
def generate(
    ...
    def draw(grid, output):
        ...
        for lrow, lcol, urow, ucol in zip(lowerrows, lowercols, upperrows, uppercols):
            dr, dc = (0 if lrow == urow else 1), (0 if lcol == ucol else 1)
            r, c = lrow, lcol
            if common.get_pixel(grid, r - dr, c - dc) == -2:
                legal = False
            while True:
                # Check for intersection.
                if common.get_pixel(grid, r, c) == -2:
                    legal = False
                if common.get_pixel(grid, r + dr, c + dc) == -2:
                    legal = False
                if grid[r][c] == 0:
                    output[r][c] = grid[r][c] = -2
                if r == urow and c == ucol:
                    break
                r, c = r + dr, c + dc
...
def validate():
    ...
    train = [
        generate(
            ...
            uppercols=[12, 21, 17, 20, 6, 10, 13, 16, 20],
            color=8,
            flip=0,
            xpose=0,
        ),
        ...
        generate(
            ...
            uppercols=[16, 17, 18, 20, 9, 4, 7, 13, 18],
            color=4,
            flip=0,
            xpose=0,
        ),
    ...
    test = [
        generate(
            ...
            uppercols=[23, 23, 19, 20, 7, 5, 11, 16, 22],
            color=7,
            flip=0,
            xpose=0,
        ),
