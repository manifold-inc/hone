...
def generate(lengths=None, talls=None, rows=None, cols=None, flip=None, size=10):
    ...
    if lengths is None:
        while True:
            ...
            talls = [
                common.randint(max(3, border - 1), border),
                common.randint(max(3, size - border - 2), size - border - 1),
            ]
            if lengths[0] != lengths[1] and talls[0] != talls[1]:
                continue  # no match
            if lengths[0] == lengths[1] and talls[0] == talls[1]:
                continue  # same!
            if lengths[0] > lengths[1] or talls[0] > talls[1]:
                continue  # red smaller
            break
