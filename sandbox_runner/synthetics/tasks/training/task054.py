...
def generate(
    ...
    def draw_star(grid, r, c, extra):
        for dr in [-1, 0, 1]:
            for dc in [-1, 0, 1]:
                if starcolors[2] != -1:
                    grid[r + dr][c + dc] = starcolors[2]
        grid[r][c] = starcolors[0]
        for dr in [-1, 1]:
            if vert:
                grid[r + dr][c] = starcolors[1]
            if vert and extra:
                grid[r + 2 * dr][c] = starcolors[1]
        for dc in [-1, 1]:
            if horiz:
                grid[r][c + dc] = starcolors[1]
            if horiz and extra:
                grid[r][c + 2 * dc] = starcolors[1]
    ...
    def draw(grid, output):
        ...
        for row, col, sidx in zip(srows, scols, sidxs):
            for c in range(bcols[sidx], bcols[sidx] + wides[sidx]):
                if horiz:
                    output[row + brows[sidx]][c] = starcolors[1]
            for r in range(brows[sidx], brows[sidx] + talls[sidx]):
                if vert:
                    output[r][col + bcols[sidx]] = starcolors[1]
        ...
        while True:
            ...
            for idx in range(num_boxes):
                ...
                while True:
                    ...
                    if num_stars == 1 or max(xrows) - min(xrows) > 5:
                        break
                while True:
                    xcols = common.sample(range(1, wides[idx] - 1), num_stars)
                    if num_stars == 1 or max(xcols) - min(xcols) > 5:
                        break
                srows.extend(xrows)
                scols.extend(xcols)
