...
def generate(colors=None, srows=None, scols=None, brows=None, bcols=None, size=10):
            ...
            for c in range(size):
                ...
                for srow, scol in zip(srows, scols):
                    if r + srow >= size or c + scol >= size or grid[r + srow][c + scol]:
                        possible = False
                if possible and (r, c) not in locations:
                    legal = False
        # Draw them red.
        for idx in range(len(brows)):
            brow, bcol = brows[idx], bcols[idx]
            for srow, scol in zip(srows, scols):
                if output[brow + srow][bcol + scol] == common.red():
                    legal = False
    ...
    if colors is None:
        while True:
            ...
            while True:
                ...
                if common.diagonally_connected(list(zip(srows, scols))):
                    break
