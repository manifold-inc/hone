...
def generate(
    rows=None, cols=None, widths=None, heights=None, colors=None, boxes=3, size=15
):
    ...
    if rows is None:
        while True:
            ...
            if common.overlaps(rows, cols, widths, heights, 2):
                continue
            ...
            if sum(w * t for w, t in zip(newwidths, newheights)) < 2 * boxes:
                continue
    ...
    for row, col, width, height, color in zip(rows, cols, widths, heights, colors):
        for r in range(row - 1, row + height + 1):
            for c in range(col - 1, col + width + 1):
                ...
                if r < row or r >= row + height or c < col or c >= col + width:
                    continue
