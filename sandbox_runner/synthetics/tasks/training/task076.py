    ...
    if width is None:
        ...
        while True:
            ...
            for megarotate in megarotates:
                ...
                megawides.append(w)
                megatalls.append(t)
            if not common.overlaps(megarows, megacols, megawides, megatalls, 1):
                break
    ...
    for i, megarotate in enumerate(megarotates):
        ...
        for row, col, color in zip(rows, cols, colors):
            ...
            if megarotate == 1:
                r, c = c, tall - 1 - r
            if megarotate == 2:
                r, c = tall - 1 - r, wide - 1 - c
            if megarotate == 3:
                c = wide - 1 - c
            output[mr + r][mc + c] = color
            if i != 0 and color not in [common.yellow(), common.red()]:
                continue
...
def validate():
    ...
    train = [
        ...
        generate(
            width=13,
            height=13,
            rows=[0, 1, 1, 1, 1, 2, 3, 3, 3, 3],
            cols=[2, 0, 1, 2, 3, 2, 0, 1, 2, 3],
            colors=[2, 4, 4, 4, 1, 4, 3, 3, 4, 3],
            megarows=[1, 4, 9],
            megacols=[2, 8, 3],
            megarotates=[0, 3, 1],
        ),
