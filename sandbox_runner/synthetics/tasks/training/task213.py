...
def generate(
    ...
    if width is None:
        ...
        if box_placement == 1:
            row = common.randint(0, height - len(colors) - 2)
        if box_placement == 2:
            col = common.randint(0, width - len(colors) - 2)
...
def validate():
    ...
    train = [
        ...
        generate(
            width=12, height=10, offset=2, row=0, col=0, xpose=1, colors=[1, 2, 4]
        ),
        generate(
            width=12, height=12, offset=1, row=0, col=5, xpose=0, colors=[2, 8, 4, 1]
        ),
    ]
    test = [
        generate(
