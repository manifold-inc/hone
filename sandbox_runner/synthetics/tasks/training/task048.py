...
def generate(width=None, height=None, rows=None, cols=None, boxrows=None, boxcols=None):
    ...
    def maybe_add(queue, visited, r, c):
        if r < 0 or r >= height or c < 0 or c >= width:
            return False
        if not grid[r][c] or (r, c) in queue or (r, c) in visited:
            return False
        queue.append((r, c))
    ...
    def path_exists(diags_allowed=False):
        ...
        queue = [
            (boxrows[0], boxcols[0]),
            (boxrows[0], boxcols[0] + 1),
            (boxrows[0] + 1, boxcols[0]),
            (boxrows[0] + 1, boxcols[0] + 1),
        ]
    ...
    if width is None:
        ...
        while True:
            ...
            lengths = [2] * num_boxes
            if common.overlaps(boxrows, boxcols, lengths, lengths, 1):
                continue
            ...
            if path_exists() == path_exists(True):
                break  # prevent ambiguous cases
...
def validate():
    ...
    train = [
        ...
        generate(
            width=7,
            height=5,
            rows=[0, 1, 1, 1, 2, 2, 3, 4, 4],
            cols=[1, 3, 4, 5, 2, 3, 2, 1, 4],
            boxrows=[1, 2],
            boxcols=[0, 5],
        ),
        generate(
            ...
            cols=[0, 3, 4, 6, 1, 2, 5, 2, 6, 0, 2, 3, 4, 0],
            boxrows=[0, 4],
            boxcols=[1, 5],
        ),
        generate(
            width=7,
            height=6,
            rows=[0, 0, 1, 1, 1, 2, 2, 3, 4, 4, 5, 5, 5],
            cols=[0, 1, 1, 2, 6, 3, 5, 0, 4, 6, 3, 4, 6],
            boxrows=[0, 4],
            boxcols=[4, 1],
        ),
        generate(
            width=7,
            height=6,
            rows=[0, 0, 1, 2, 3, 3, 4, 4, 5, 5],
            cols=[0, 5, 5, 0, 2, 5, 2, 6, 0, 5],
            boxrows=[1, 4],
            boxcols=[2, 3],
        ),
        generate(
            width=6,
            height=6,
            rows=[0, 0, 1, 1, 2, 3, 3, 4, 5, 5, 5],
            cols=[0, 5, 0, 2, 4, 2, 4, 5, 1, 2, 4],
            boxrows=[0, 3],
            boxcols=[3, 0],
        ),
    ]
