...
def generate(width=None, height=None, rows=None, cols=None, lines=None, xpose=None):
    ...
    if width is None:
        while True:
            ...
            if num_lines == 1:
                ...
                lines = [
                    common.randint(height // 5, 2 * height // 5),
                    common.randint(3 * height // 5, 4 * height // 5),
                ]
            rows, cols = [], []
            for c in range(1, width - 1):
                r = common.randint(0, height - 1)
                if r in lines or r - 1 in lines or r + 1 in lines:
                    continue
                if cols and cols[-1] + 4 > c:  # Need to ensure spacing between boxes.
                    if num_lines == 1:
                        continue
                    if min(r, rows[-1]) > lines[0] or max(r, rows[-1]) < lines[1]:
                        continue
                if common.randint(0, 2):
                    continue
                rows.append(r)
                cols.append(c)
...
def validate():
    ...
    train = [
        generate(width=13, height=13, rows=[2, 10], cols=[2, 8], lines=[6], xpose=0),
        generate(width=13, height=18, rows=[8], cols=[4], lines=[3, 14], xpose=1),
        generate(
            width=12, height=17, rows=[2, 10], cols=[8, 4], lines=[7, 13], xpose=0
        ),
    ]
    test = [
        generate(
