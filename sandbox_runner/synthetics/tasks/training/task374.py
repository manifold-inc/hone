...
def generate(rows=None, cols=None, lengths=None, ups=None, size=10, colors=(2, 4, 1)):
            ...
            for j in range(len(lengths)):
                for i in range(len(lengths)):
                    ...
                    if ups[i] == 1 and ups[j] == 0:  # Don't need to handle fourth case!
                        if rows[i] > rows[j] + 1:
                            continue
                        if rows[i] + lengths[i] + 1 < rows[j]:
                            continue
                        if cols[i] + 1 < cols[j]:
                            continue
                        if cols[j] + lengths[j] + 1 < cols[i]:
                            continue
                        overlaps = True
            if not overlaps:
                break
...
def validate():
    ...
    train = [
        generate(rows=[4, 2, 1], cols=[7, 4, 1], lengths=[3, 5, 6], ups=[1, 1, 1]),
        generate(rows=[5, 3, 1], cols=[7, 1, 4], lengths=[2, 4, 6], ups=[1, 1, 1]),
        generate(rows=[7, 3, 2], cols=[2, 7, 0], lengths=[3, 5, 6], ups=[0, 1, 0]),
        generate(rows=[2, 5, 1], cols=[1, 1, 7], lengths=[4, 5, 7], ups=[0, 0, 1]),
    ]
    test = [
        generate(rows=[1, 8, 5], cols=[3, 0, 4], lengths=[3, 5, 6], ups=[1, 0, 0]),
    ]
