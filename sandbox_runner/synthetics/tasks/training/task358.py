...
def generate(
    width=None, height=None, row=None, col=None, colors=None, flip=None, offset=None
):
    ...
    for r in range(height):
        for c in range(width):
            ...
            if r + offset < row or r + offset >= row + len(colors):
                continue
            if c + offset < col or c + offset >= col + len(colors):
                continue
...
def validate():
    ...
    train = [
        generate(width=11, height=12, row=4, col=2, colors=[2, 8, 3], flip=0, offset=2),
        generate(
            width=14, height=15, row=6, col=5, colors=[2, 4, 8, 3], flip=1, offset=2
        ),
    ]
    test = [
        generate(
            width=20, height=20, row=6, col=6, colors=[2, 3, 1, 4], flip=0, offset=1
        ),
    ]
