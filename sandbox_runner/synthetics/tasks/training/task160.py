...
def generate(rows=None, cols=None, idxs=None, size=10, num=5):
    ...
    def draw_sprite(bitmap, r, c, idx, color):
        ...
        p = (
            p
            if idx != 0
            else [(0, 0), (0, 1), (0, 2), (1, 0), (1, 2), (2, 0), (2, 1), (2, 2)]
        )
    ...
    def draw(grid, output, sprite_grid):
        ...
        for r in range(size):
            for c in range(size):
                if not sprite_grid[r][c]:
                    continue
                for dr, dc in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
                    color = common.get_pixel(sprite_grid, r + dr, c + dc)
                    if color not in [-1, 0, sprite_grid[r][c]]:
                        return False
        return True
    ...
    if rows is None:
        while True:
            ...
            if 0 not in idxs:
                continue  # We need at least one box to convert!
            overlaps = False
            ...
            if not draw(grid, output, sprite_grid):
                continue
            if not overlaps:
                break
...
def validate():
    ...
    train = [
        generate(rows=[0, 1, 4, 6, 5], cols=[0, 6, 0, 3, 7], idxs=[0, 1, 1, 0, 2]),
        generate(rows=[0, 1, 3, 5, 7], cols=[4, 0, 7, 1, 6], idxs=[0, 3, 1, 0, 4]),
    ]
    test = [
        generate(rows=[0, 2, 4, 7, 7], cols=[7, 1, 5, 1, 7], idxs=[1, 0, 2, 0, 5]),
    ]
