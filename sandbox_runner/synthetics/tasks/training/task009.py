...
def generate(size=None, rows=None, cols=None, colors=None, linecolor=None):
    ...
    if size is None:
        ...
        pixels = [
            [row_gap, col_gap],
            [row_gap + height - 1, col_gap + width - 1],
            [row_gap, col_gap + width - 1],
            [row_gap + height - 1, col_gap],
        ]
        ...
        for _ in range(2):
            ...
            if common.randint(0, 1) == 0:  # horiz
                ...
                col.sort()
                if not horiz_free(bitmap, row, col):
                    continue
                ...
                row.sort()
                if not vert_free(bitmap, col, row):
                    continue
                newrows.extend(row)
        ...
        while True:
            ...
            if matches_up(rows, cols, colors, row, col, color):
                continue
            rows.append(row)
            cols.append(col)
            colors.append(color)
...
def validate():
    ...
    train = [
        ...
        generate(
            size=8,
            rows=[1, 1, 3, 3, 4, 5, 6, 6],
            cols=[1, 4, 3, 7, 6, 1, 6, 3],
            colors=[2, 4, 9, 9, 8, 2, 8, 8],
            linecolor=1,
        ),
        generate(
            size=8,
