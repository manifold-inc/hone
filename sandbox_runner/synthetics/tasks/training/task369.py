...
def generate(colors=None, size=10):
    ...
    if colors is None:
        ...
        for r in range(size):
            for c in range(size):
                if not common.randint(0, 5):
                    continue
                num_pixels, pixels = common.randint(1, 4), []
                dr, dc = (0, 1) if common.randint(0, 1) else (1, 0)
                if num_pixels in [3, 2, 1]:
                    pixels.append((r, c))
                if num_pixels in [3, 2]:
                    pixels.append((r + dr, c + dc))
                if num_pixels in [3]:
                    pixels.append((r + 2 * dr, c + 2 * dc))
                ...
                for r, c in pixels:
                    if r >= size or c >= size:
                        overlaps = True
                        continue
                    if r > 0 and bitmap[r - 1][c] != common.gray():
                        overlaps = True
                    if r + 1 < size and bitmap[r + 1][c] != common.gray():
                        overlaps = True
                    if c > 0 and bitmap[r][c - 1] != common.gray():
                        overlaps = True
                    if c + 1 < size and bitmap[r][c + 1] != common.gray():
                        overlaps = True
                if overlaps:
                    continue
                for r, c in pixels:
