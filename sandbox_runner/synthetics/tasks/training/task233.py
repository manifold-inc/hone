...
def generate(
    ...
    if width is None:
        ...
        while True:
            ...
            lens = [3] * num_sprites
            if common.overlaps(
                srows + [brow], scols + [bcol], lens + [wide], lens + [tall], 1
            ):
                continue
            if common.overlaps(irows, icols, lens, lens, 0):
                continue
            ...
            for idx in range(num_sprites):
                while True:  # Avoid sprites where rotation might be ambiguous.
                    ...
                    for c in range(3):
                        ...
                        if blank:
                            num_blank += 1
                    if counts[idx] == 4 and num_blank >= 2:
                        continue  # Avoid 2x2
                    if counts[idx] == 6 and num_blank >= 1:
                        continue  # Avoid 2x3
                    break
...
def validate():
    ...
    train = [
        generate(
            width=19,
            height=24,
            rows=[0, 1, 1, 1, 2, 1, 1, 1, 1, 1, 2, 2, 2, 1, 2, 2, 0, 1, 1, 1, 2],
            cols=[1, 0, 1, 2, 1, 0, 1, 2, 0, 2, 0, 1, 2, 1, 0, 1, 2, 0, 1, 2, 2],
            idxs=[0, 0, 0, 0, 0, 1, 1, 1, 2, 2, 2, 2, 2, 3, 3, 3, 4, 4, 4, 4, 4],
            srows=[2, 7, 17, 20, 20],
            scols=[13, 14, 14, 3, 8],
            irows=[6, 1, 13, 1, 13],
            icols=[3, 6, 0, 1, 5],
