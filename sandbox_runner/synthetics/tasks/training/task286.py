...
def generate(width=None, height=None, colors=None):
        ...
        while True:
            ...
            for r in range(height):
                for c in range(width):
                    if r % 2 != rmod or c % 2 != cmod:
                        continue
                    viable = True
                    for dr in [-1, 0, 1]:
                        for dc in [-1, 0, 1]:
                            if not common.get_pixel(grid, r + dr, c + dc):
                                viable = False
                    if not viable:
                        continue
                    rows.append(r)
            ...
            while queue:
                ...
                r, c = queue[i]
                del queue[i]
                if not common.randint(0, 2):
                    continue  # Sometimes we trim paths.
                for dr, dc in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
                    if common.get_pixel(grid, r + dr, c + dc) in [0, -1]:
                        continue
                    if common.get_pixel(grid, r + 2 * dr, c + 2 * dc) in [0, -1]:
                        continue
    ...
    while queue:
        r, c = queue.pop()
        if r in [-1, height] or c in [-1, width] or output[r][c]:
            continue
        for dr, dc in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
