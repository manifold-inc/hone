...
def generate(
    ...
    if width is None:
        ...
        while True:
            ...
            if redrow + tall < cyanrow:
                break
        redcol = common.randint(0, width - wide)
        cyancol = common.randint(max(0, redcol - 1), min(width - 2, redcol + wide - 1))
        while True:  # Check that the nibbles don't just form a smaller box.
            ...
            for row in range(tall):
                if len([r for r in rows if r == row]) == wide:
                    all_gone = True
            for col in range(wide):
                if len([c for c in cols if c == col]) == tall:
                    all_gone = True
            if not all_gone:
                break
