...
def generate(colors=None, size=18):
            ...
            if stripe == 0:  # First, paint all downstripes
                for r in range(size - 1):
                    for c in range(size - 1):
                        ...
                        if (
                            c < size - 2
                            and output[r][c + 2] + output[r + 1][c + 2] == 0
                        ):
                            legal = False
                            continue
                        topaint.append((r, c))
            if stripe == 1:  # Second, paint all sidestripes
                for r in range(size - 1):
                    for c in range(size - 1):
                        ...
                        if (
                            r < size - 2
                            and output[r + 2][c] + output[r + 2][c + 1] == 0
                        ):
                            legal = False
                            continue
                        topaint.append((r, c))
    ...
    if colors is None:
        while True:
            ...
            if not draw(grid, output1, [0, 1]):
                continue
            grid, output2 = common.grids(size, size)
            if not draw(grid, output2, [1, 0]):
                continue
            if output1 == output2:
                break  # Avoid ambigous problems.
