...
def generate(
    width=None, rows=None, cols=None, colors=None, quadrant=None, height=2, size=9
):
    ...
    if rows is None:
        ...
        color_list = common.random_colors(
            common.randint(3, 4), exclude=[common.yellow()]
        )
    ...
    for r in range(height * (size + 1) - 1):
        for c in range(width * (size + 1) - 1):
            if r % (size + 1) != size and c % (size + 1) != size:
                continue
...
def validate():
    ...
    train = [
        generate(
            width=2,
            rows=[1, 2, 3, 4, 4, 6, 6, 7, 7, 8],
            cols=[5, 3, 2, 1, 5, 3, 4, 4, 7, 1],
            colors=[7, 2, 2, 3, 3, 8, 7, 8, 3, 7],
            quadrant=2,
        ),
        generate(
            width=1,
            rows=[1, 1, 2, 4, 5, 6, 6, 7, 7, 7],
    ...
    test = [
        generate(
            width=3,
            rows=[1, 2, 2, 3, 3, 4, 6, 8],
            cols=[4, 1, 7, 1, 7, 4, 2, 6],
            colors=[3, 2, 2, 3, 2, 2, 6, 6],
            quadrant=1,
        ),
    ]
