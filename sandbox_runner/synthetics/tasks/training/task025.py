    ...
    if width is None:
        ...
        for color in linecolors:
            ...
            for i, pixel in enumerate(pixels):
                if linecolors[color_idxs[i]] != color:
                    continue
                if pixel[0] in rows_taken or pixel[1] in linecols:
                    continue
                if pixel[1] - 1 in linecols or pixel[1] + 1 in linecols:
                    continue
                rows.append(pixel[0])
                cols.append(pixel[1])
                colors.append(color)
