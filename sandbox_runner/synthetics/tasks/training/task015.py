...
def generate(rows=None, cols=None, colors=None, size=9, twinklers=(1, 2)):
    ...
    if rows is None:
        ...
        for idx in range(len(pixels)):
            if colors[idx] not in twinklers:
                continue
        ...
        for idx in range(len(pixels)):
            ...
            for prev in range(idx):
                ...
                if abs(rows[idx] - rows[prev]) > radius:
                    continue
                if abs(cols[idx] - cols[prev]) > radius:
                    continue
                good = False
            if good:
                keep.append(idx)
...
def validate():
    ...
    train = [
        ...
        generate(rows=[0, 2, 3, 6, 7], cols=[3, 6, 2, 6, 1], colors=[8, 2, 1, 1, 2]),
        generate(rows=[2, 5, 7], cols=[2, 6, 3], colors=[2, 6, 1]),
    ]
    test = [
        generate(rows=[2, 3, 5, 7, 7], cols=[6, 2, 5, 7, 1], colors=[1, 2, 8, 2, 6]),
    ]
