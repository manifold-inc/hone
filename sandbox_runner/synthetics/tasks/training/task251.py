...
def generate(size=None, rows=None, cols=None, wides=None, talls=None):
    ...
    def draw(grid, output):
        ...
        if grid == output:
            return False  # Need to draw at least one blue.
            ...
            for col in range(size):
                ...
                for wide in range(3, size - col):
                    for tall in range(3, size - row):
                        is_rect = True
                        for r in range(row + 1, row + tall - 1):
                            if grid[r][col] != common.red():
                                is_rect = False
                            if grid[r][col + wide - 1] != common.red():
                                is_rect = False
                        for c in range(col + 1, col + wide - 1):
                            if grid[row][c] != common.red():
                                is_rect = False
                            if grid[row + tall - 1][c] != common.red():
                                is_rect = False
                        if is_rect:
                            return False
        return True
    ...
    if size is None:
        ...
        while True:
            ...
            if common.overlaps(rows, cols, wides, talls, -1):
                continue
...
def validate():
    ...
    test = [
        generate(
            size=9, rows=[0, 3, 6], cols=[2, 0, 4], wides=[5, 5, 6], talls=[4, 5, 4]
        ),
    ]
