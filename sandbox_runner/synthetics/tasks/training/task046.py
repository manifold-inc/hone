...
def generate(
    rows=None, cols=None, wides=None, offsets=None, colors=None, extra=None, height=3
):
            ...
            for i in range(w):
                ...
                if i == turn:
                    ...
                    while True:  # Turn as much as desired, but don't go off the grid.
                        ...
                        rows.append(r)
                        cols.append(c)
                        if r in [0, height - 1] or common.randint(0, 1) or w > 2:
                            break
                c += 1
        offsets, c = [], 0
    ...
    for i, idx in enumerate(idxs):
        ...
        if i > 0 and idx != idxs[i - 1]:
            color = common.gray()
        if i + 1 < len(idxs) and idx != idxs[i + 1]:
            color = common.gray()
...
def validate():
    ...
    test = [
        generate(
            rows=[1, 1, 0, 0, 0, 1, 2, 2, 2, 1, 1, 1, 2],
