...
def generate(rows=None, cols=None, idxs=None, colors=None, size=10, minisize=3):
    ...
    if rows is None:
        ...
        while True:
            ...
            if abs(rows[0] - rows[1]) > 2 and abs(cols[0] - cols[1]) > 2:
                break
        while True:
            ...
            if not common.diagonally_connected(one):
                continue
            if not common.diagonally_connected(two):
                continue
...
def validate():
    ...
    train = [
        generate(
            rows=[2, 7], cols=[1, 7], idxs=[0, 0, 1, 0, 1, 1, 0, 1, 1], colors=[3, 7]
        ),
        generate(
            rows=[-1, 2], cols=[8, 3], idxs=[1, 1, 1, 0, 1, 1, 0, 0, 1], colors=[4, 6]
        ),
        generate(
            rows=[3, 8], cols=[3, 1], idxs=[1, 1, 1, 1, 0, 1, 0, 0, 0], colors=[3, 1]
        ),
    ]
    test = [
        generate(
            rows=[2, 6], cols=[2, 7], idxs=[1, 1, 0, 1, 0, 0, 1, 1, 1], colors=[2, 8]
        ),
    ]
