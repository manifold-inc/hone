...
def generate(cols=None, wides=None, talls=None, colors=None, size=15, boxes=3):
    ...
    if cols is None:
        while True:
            ...
            if not common.overlaps(cols, cols, wides, wides, 1):
                break
...
def validate():
    ...
    train = [
        generate(cols=[1, 4, 9], wides=[2, 4, 4], talls=[4, 2, 4], colors=[1, 2, 4]),
        generate(cols=[1, 7, 11], wides=[4, 2, 2], talls=[6, 2, 5], colors=[4, 1, 2]),
        generate(cols=[1, 7, 11], wides=[4, 1, 2], talls=[1, 4, 3], colors=[2, 1, 4]),
    ]
    test = [
        generate(cols=[0, 5, 10], wides=[4, 3, 5], talls=[7, 6, 3], colors=[2, 4, 1]),
    ]
