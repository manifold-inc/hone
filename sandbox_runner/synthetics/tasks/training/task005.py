...
def generate(
    ...
    if rows is None:
        ...
        if common.randint(0, 1):
            sprite[1][1] = 0  # Remove the middle pixel
        if common.randint(0, 1):  # Remove one side cell
            side = common.randint(0, 3)
            if side == 0:
                sprite[0][1], rbad, cbad = 0, 1, 0
            if side == 1:
                sprite[1][2], rbad, cbad = 0, 0, -1
            if side == 2:
                sprite[2][1], rbad, cbad = 0, -1, 0
            if side == 3:
                sprite[1][0], rbad, cbad = 0, 0, 1
        else:  # If we didn't remove a side, maybe remove some (or both) diagonals
            if common.randint(0, 1):
                sprite[0][0] = sprite[2][2] = 0
            if common.randint(0, 1):
                sprite[0][2] = sprite[2][0] = 0
        for r in range(3):
            for c in range(3):
                if sprite[r][c]:
                    rows, cols = rows + [r], cols + [c]
    ...
    for rdir, cdir, color in zip(rdirs, cdirs, colors):
        ...
        while True:
            ...
            first_sprite = False
            if middle_sprite:
                break  # It's the middle sprite
            if row < -5 or row > size or col < -5 or col > size:
                break
