...
def generate(
    width=None, height=None, rows=None, cols=None, wides=None, talls=None, colors=None
):
    ...
    if width is None:
        ...
        while True:
            ...
            if max(wides) > width - 2 or max(talls) > height - 2:
                continue  # Too big.
...
def validate():
    ...
    train = [
        ...
        generate(
            width=7, height=8, rows=[1], cols=[1], wides=[5], talls=[4], colors=[2]
        ),
        generate(
            width=12,
            height=11,
