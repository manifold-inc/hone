...
def generate(width=None, height=None, colors=None, rows=None, cols=None, size=10):
    ...
    if width is None:
        ...
        while True:
            ...
            for j in range(num_sprites):
                for i in range(j):
                    if abs(rows[j] - rows[i]) > height:
                        continue
                    if abs(cols[j] - cols[i]) > width:
                        continue
                    overlaps = True
            if not overlaps:
                break
...
def validate():
    ...
    train = [
        ...
        generate(
            width=4,
            height=3,
            colors=[6, 6, 6, 6, 8, 8, 6, 8, 6, 8, 8, 8],
            rows=[1, 0, 5],
            cols=[1, 6, 4],
        ),
    ]
    test = [
        generate(
            width=3,
            height=4,
            colors=[4, 4, 4, 1, 4, 4, 1, 4, 1, 1, 1, 1],
            rows=[0, 1, 5, 6],
            cols=[1, 6, 2, 7],
        ),
    ]
