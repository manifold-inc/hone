...
def generate(length=None, colors=None, hflip=None, vflip=None, size=19):
    ...
    if length is None:
        ...
        for i in range(length):
            if i + 1 == length and common.randint(0, 1):
                continue  # Maybe skip last.
            bitmap[i][i] = color_list[idx]
            idx += 1
            if i + 1 == length:
                continue  # No room to add a nextdoor color.
            if i and bitmap[i][i - 1]:
                continue  # Avoid three colors in a row.
            if common.randint(0, 1):
                continue  # Sometimes don't add a nextdoor color.
    ...
    for r in range(length):
        for c in range(length):
            ...
            if r != c or r + 1 == length:
                continue
            if not colors[r * length + c + 1]:
                continue
...
def validate():
    ...
    test = [
        generate(
            length=4,
            colors=[1, 3, 0, 0, 3, 2, 0, 0, 0, 0, 4, 8, 0, 0, 8, 0],
            hflip=0,
            vflip=1,
        ),
    ]
